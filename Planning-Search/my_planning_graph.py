...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for act in self.all_actions:
            add_nodeA = PgNode_a(act)
            if add_nodeA.prenodes.issubset(self.s_levels[level]):
                Anodes.append(add_nodeA)
                for nodeS in self.s_levels[level]:
                    nodeS.children.add(add_nodeA)
                    add_nodeA.parents.add(add_nodeA)
    ...
    def add_literal_level(self, level):
        ...
        for node_a in self.a_levels[level - 1]:
            for nodeS in node_a.effnodes:
                Snodes.add(nodeS)
                nodeS.parents.add(node_a)
                node_a.children.add(nodeS)
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for effect_a1 in node_a1.parents:
            for effect_a2 in node_a2.parents:
                if effect_a1.is_mutex(effect_a2):
                    return True
        return False
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        for s1_literal in node_s1.parents:
            for s2_literal in node_s2.parents:
                if not s1_literal.is_mutex(s2_literal):
                    return False
        return True
